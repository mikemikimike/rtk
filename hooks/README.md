...
Owns: per-agent hook scripts and configuration files for supported agents (Claude Code, Copilot, Cursor, Cline, Windsurf, Codex, OpenCode, MiniMax Code, Hermes, Pi, Mistral Vibe).
...
- **[`mimocode/`](mimocode/README.md)** — MiniMax Code TypeScript plugin, `tool.execute.before` event, in-place mutation
...
| MiniMax Code | TypeScript plugin (`tool.execute.before`) | In-place mutation | Yes |
